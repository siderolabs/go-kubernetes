...
#
# Generated on 2026-07-15T17:37:59Z by kres d66531d.
...
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-kubernetes/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-kubernetes/go/pkg go install github.com/shanduur/dis-vulncheck@${DIS_VULNCHECK_VERSION} \
...
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-kubernetes/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-kubernetes/go/pkg GOARCH=amd64 GOOS=darwin go test -c -covermode=atomic -coverpkg=github.com/siderolabs/go-kubernetes/... -tags integration,sidero.debug -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /integration-test-darwin-amd64
...
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-kubernetes/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-kubernetes/go/pkg GOARCH=arm64 GOOS=darwin go test -c -covermode=atomic -coverpkg=github.com/siderolabs/go-kubernetes/... -tags integration,sidero.debug -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /integration-test-darwin-arm64
...
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-kubernetes/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-kubernetes/go/pkg GOARCH=amd64 GOOS=linux go test -c -covermode=atomic -coverpkg=github.com/siderolabs/go-kubernetes/... -tags integration,sidero.debug -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /integration-test-linux-amd64
...
ARG GO_GCFLAGS
ARG GO_LDFLAGS
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-kubernetes/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-kubernetes/go/pkg GOARCH=arm64 GOOS=linux go test -c -covermode=atomic -coverpkg=github.com/siderolabs/go-kubernetes/... -tags integration,sidero.debug -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS}" -o /integration-test-linux-arm64
...
WORKDIR /src
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-kubernetes/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-kubernetes/go/pkg dis-vulncheck -tool=false ./...
