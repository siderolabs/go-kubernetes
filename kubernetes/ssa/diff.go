...
func (m *Manager) Diff(ctx context.Context, objects []*unstructured.Unstructured, ops DiffOptions) ([]DiffResult, error) {
	...
	if !ops.NoPrune {
		...
		for _, objMeta := range pruneObjRefs {
			...
			result = append(result, DiffResult{
				...
				Subject:      formatPath(obj.GetNamespace(), obj.GetName(), obj.GetKind()),
			})
		}
	}
	...
	for _, obj := range objects {
		...
		changeSet, diff, err := m.diff(ctx, obj, ops.Force, ops.InventoryPolicy, inv.ID())
		if err != nil {
			return nil, err
		}
	...
	if err != nil && (apierrors.IsNotFound(err) || strings.Contains(err.Error(), "not found")) {
		if changeSet == nil {
			changeSet = &ssa.ChangeSetEntry{
				...
				GroupVersion: inputObj.GetAPIVersion(),
